...
import type {DisplayDepth,Point,Scene,Tour} from "@/lib/imo3d/model";
...
type ViewerTour=Tour&{media?:ViewerMedia;deferredDepth?:string[]};
/** Signed-in administrators preview published tours; only they are offered the way back to the studio. */
function useStudioAdmin(){
  const [admin,setAdmin]=useState(false);
  useEffect(()=>{let active=true;api<{admin?:boolean}>("session").then(session=>{if(active)setAdmin(session.admin===true);}).catch(()=>{});return()=>{active=false;};},[]);
  return admin;
}
...
export default function TourViewer({id,embedded=false,initialSceneId}:{id:string;embedded?:boolean;initialSceneId?:string}) {
  ...
  const admin=useStudioAdmin(),studio=studioLink({embedded,admin});
  ...
  if(error)return <main className="imo-shell imo-load"><Icon name="info" size={32}/><h1>تعذر فتح الجولة</h1><p>{error}</p>{studio?<a className="imo-button primary" href={studio}>العودة إلى الاستوديو</a>:<button type="button" className="imo-button primary" onClick={()=>window.location.reload()}>إعادة المحاولة</button>}</main>;
  if(!tour)return <main className="imo-shell imo-load"><span className="imo-spinner"/><p>جارٍ تحضير جولتك…</p></main>;
  if(!tour.scenes.length)return <main className="imo-shell imo-load"><h1>{tour.title}</h1><p>لم تُضف لقطات لهذه الجولة بعد.</p>{studio&&<a href={studio} className="imo-button primary">فتح الاستوديو</a>}</main>;
  return <Viewer key={tour.id} tour={tour} studio={studio} initialSceneId={tour.scenes.some(scene=>scene.id===initialSceneId)?initialSceneId:undefined}/>;
}
function Viewer({tour,studio,initialSceneId}:{tour:ViewerTour;studio:string|null;initialSceneId?:string}) {
  ...
  // Display depth is withheld from a buyer's arrival payload; each scene's is fetched with its panorama.
  const depthRequests=useRef(new Map<string,Promise<DisplayDepth|undefined>>());
  const [,setDepthArrivals]=useState(0);
  const sceneDepth=useCallback((scene:Scene):Promise<DisplayDepth|undefined>=>{
    const target=scenesById.current.get(scene.id);
    if(!target||target.displayDepth||!tour.deferredDepth?.includes(scene.id))return Promise.resolve(target?.displayDepth);
    let request=depthRequests.current.get(scene.id);
    if(!request){
      request=fetch(withBasePath(`/api/imo3d/tours/${encodeURIComponent(tour.id)}/depth/${encodeURIComponent(scene.id)}?r=${tour.revision}`)).then(async response=>{
        if(!response.ok)throw Error("DEPTH_UNAVAILABLE");
        const depth=await response.json() as DisplayDepth;
        if(!target.displayDepth)target.displayDepth=depth;
        if(currentRef.current.id===scene.id)setDepthArrivals(count=>count+1);
        return depth;
      }).catch(()=>{depthRequests.current.delete(scene.id);return undefined;});
      depthRequests.current.set(scene.id,request);
    }
    return request;
  },[tour.id,tour.revision,tour.deferredDepth]);
  const sceneDepthRef=useRef(sceneDepth);
  useEffect(()=>{sceneDepthRef.current=sceneDepth;},[sceneDepth]);
  ...
  async function navigate(target:Scene,walk=true,entry=false) {
    ...
    navigating.current=true;setSelectedMeasurement(null);setError("");setMeasure(false);measureRef.current=false;setPoints([]);setFloorRays([]);if(photoCalibration.current?.source==="vertical_height")photoCalibration.current={...photoCalibration.current,base:undefined};photoCalibration.current=null;setNotice("");
    try{
  ...
  useEffect(()=>{
    ...
    void import("./PanoramaEngine").then(async({PanoramaEngine})=>{
      if(cancelled)return;instance=new PanoramaEngine(el,{plans:tour.plans,sceneDepth:scene=>sceneDepthRef.current(scene),resolveAsset:url=>media.current&&Date.now()<media.current.expiresAt?media.current.urls[url]??url:url});engine.current=instance;
    ...
    const pointerUp=(e:PointerEvent)=>{pointers.delete(e.pointerId);if(!down||!instance)return;
      ...
      if(measureRef.current){
        ...
        if(!planeMeasureRef.current&&((scene.depth&&tour.spatialScale==="metric")||supportedDisplayDepth(scene.displayDepth))){
          ...
          if(!local){setNotice(calibration?.source==="vertical_height"?"اختر أسفل الباب عند الأرض، ثم أعلاه على الحافة الرأسية نفسها (فوق النقطة الأولى مباشرة).":calibration?.source==="ceiling_height"?"اختر نقطة على السقف المحدد فوق الأفق.":calibration?.source==="wall_reference"||calibration?.source==="wall_height"?"اختر نقطة واضحة على الجدار الذي حددته.":"اختر نقطة واضحة على الأرض أسفل الأفق.");return;}
          const origin=scene.position??{x:0,y:0,z:0},point={x:origin.x+local.x,y:origin.y+local.y,z:origin.z+local.z};
          if(calibration?.source==="vertical_height")photoCalibration.current={...calibration,base:calibration.base?undefined:ray};
  ...
  return <div className={`imo-shell imo-viewer${controlsHidden?" imo-viewer--controls-hidden":""}`} ref={container} data-mobile-map={mobileMapVisible?"shown":"hidden"} style={{"--imo-accent":accent,"--imo-accent-ink":accentInk} as CSSProperties}>
    ...
    {!measure&&panel!=="map"&&(mobile?mobileMapVisible:mapVisible)&&<aside className="imo-under-logo-map"><ViewerFloorPlan cache={planCache} onIntent={warmTarget} scenes={tour.scenes} yaw={yaw} key={`${tour.id}/${current.floor}`} tourId={tour.id} floor={current.floor} compact hasInteractivePlan={defaultPlanView(currentPlan)==="architecture"} current={current.id} sceneIds={tour.scenes.filter(s=>s.floor===current.floor).map(s=>s.id)} onSelect={id=>{const target=scenesById.current.get(id);if(target)void navigate(target,false);}} onExpand={()=>{setFloor(current.floor);openPanel("map");}}/><UnitSummary tour={tour}/></aside>}
    ...
    {measure&&<><div className="imo-measure-tools" role="toolbar" aria-label="أدوات القياس">
      <button type="button" title="إضافة قياس جديد" aria-label="إضافة قياس جديد" onClick={()=>{setMeasurementsVisible(true);pointsRef.current=[];setPoints([]);setFloorRays([]);if(photoCalibration.current?.source==="vertical_height")photoCalibration.current={...photoCalibration.current,base:undefined};setSelectedMeasurement(null);}}><Icon name="plus"/></button>
      <button type="button" title="حذف القياس المحدد" aria-label="حذف القياس المحدد" disabled={!selectedMeasurement&&points.length===0} onClick={()=>{if(selectedMeasurement)notebook.remove(selectedMeasurement);setSelectedMeasurement(null);pointsRef.current=[];setPoints([]);setFloorRays([]);if(photoCalibration.current?.source==="vertical_height")photoCalibration.current={...photoCalibration.current,base:undefined};}}><Icon name="trash"/></button>
      <button type="button" className="imo-measure-clear-all" title="حذف كل القياسات" aria-label="حذف كل القياسات" disabled={!notebook.measurements.length&&!points.length} onClick={()=>{if(!window.confirm("حذف جميع قياسات هذه الجولة؟ لا يمكن التراجع عن الحذف."))return;notebook.clear();setSelectedMeasurement(null);pointsRef.current=[];setPoints([]);setFloorRays([]);if(photoCalibration.current?.source==="vertical_height")photoCalibration.current={...photoCalibration.current,base:undefined};}}><Icon name="trash"/><small>الكل</small></button>
    ...
    </div>{(planeMeasure||(!(current.depth&&tour.spatialScale==="metric")&&!supportedDisplayDepth(current.displayDepth)))&&<PanoramaMeasurementControls fixedHeight unit={measurementUnit} origin={current.position??{x:0,y:0,z:0}} onSave={item=>{notebook.save(item);setSelectedMeasurement(item.id);pointsRef.current=[];setPoints([]);setFloorRays([]);if(photoCalibration.current?.source==="vertical_height")photoCalibration.current={...photoCalibration.current,base:undefined};}} key={current.id} sceneId={current.id} initialCalibration={measurementSeed??calibratePanoramaHeight(current.id,recordedMeasurementHeight(tour,current.id)??DEFAULT_CAPTURE_HEIGHT_METERS)} points={floorRays} markers={markers} onCalibrationChange={value=>{photoCalibration.current=value;if(value)measurementCalibrations.current.set(current.id,value);else measurementCalibrations.current.delete(current.id);}} onClear={()=>{setFloorRays([]);if(photoCalibration.current?.source==="vertical_height")photoCalibration.current={...photoCalibration.current,base:undefined};setPoints([]);}} onClose={()=>{setMeasure(false);setFloorRays([]);if(photoCalibration.current?.source==="vertical_height")photoCalibration.current={...photoCalibration.current,base:undefined};setPoints([]);photoCalibration.current=null;}}/>}</>}
    ...
    <nav className="imo-mobile-dock" aria-label="أدوات الجولة">
      ...
      <button className="imo-dock-interest" aria-haspopup="dialog" aria-expanded={panel==="lead"} onClick={()=>openPanel("lead")}><span>سجّل اهتمامك</span><Icon name="arrow" size={18}/></button>
    </nav>
