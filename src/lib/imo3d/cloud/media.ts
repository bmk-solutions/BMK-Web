...
export async function tourMedia(tour:Tour){
  const references=new Set(tour.scenes.flatMap(scene=>scene.presentation?[scene.presentation.image,scene.presentation.preview,scene.presentation.thumbnail]:[scene.image,scene.preview,scene.thumbnail,scene.detail?.image]).filter(Boolean));
