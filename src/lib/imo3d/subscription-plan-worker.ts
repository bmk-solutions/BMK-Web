...
const imageReviewSchema=z.object({imagePath:z.string().min(1).max(2000),labels:planLabelsSchema,baseImageHasNoText:z.literal(true),reviewNotes:z.string().min(10).max(6000),audit:floorplanAuditSchema}).strict();
 ...
 do{
  ...
  try{
   ...
   for(const floor of analysis.floors){
    ...
    drafts.push({id:draftId,floor:floor.floor,result:{source:'chatgpt-subscription-local',geometryBasis:floor.geometryBasis,geometryExplanation:floor.geometryExplanation,layout:floor.layout,evidence:floor.evidence,audit:reviewed.audit,furnished:{styleVersion:'photo-furnished-v2-editable-labels',parentDraftId:draftId,imageDraftId:draftId,labels:reviewed.labels,width:meta.width,height:meta.height,reviewNotes:reviewed.reviewNotes,reviewStatus:'needs-review',baseImageHasNoText:true}}});
   }
