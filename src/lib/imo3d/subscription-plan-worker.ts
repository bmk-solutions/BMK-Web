 ...
 do{
  ...
  try{
   ...
   for(const floor of analysis.floors){
    const guide=path.join(directory,`guide-${floor.floor}.png`);await sharp(Buffer.from(renderFloorplanLayoutSVG(floor.layout,{solidWalls:true}))).png().toFile(guide);
    ...
    const renderKey=`render-v3-${floor.floor}-`+createHash('sha256').update(JSON.stringify(floor)).digest('hex').slice(0,16);
