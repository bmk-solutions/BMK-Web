...
export function ChatGPTDrafts({tourId}:{tourId:string}){
 ...
 return <section><h4>مسودات ChatGPT</h4><p>صور الشقة ← مراجعة الغرف والأثاث ← مخطط 2D مفروش للمراجعة. لكل شقة توزيعها وأثاثها الخاص.</p>{!drafts.length&&<p>اضغط «تحليل الصور وإنشاء مخطط». تظهر هنا مسودات هذه الجولة عند اكتمالها.</p>}{error&&<p role="status">{error}</p>}{drafts.map(draft=><DraftCard key={draft.id} draft={draft} tourId={tourId} onSaved={()=>setRevision(value=>value+1)}/>)}</section>;
}
