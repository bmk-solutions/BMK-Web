...
const json = (body: unknown, status = 200) => Response.json(servePayloadPaths(body), { status, headers: { "Cache-Control": "no-store" } });
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const admin = !authorization && await isAdmin(request);
