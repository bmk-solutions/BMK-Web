...
function DraftCard({draft,tourId,onSaved}:{draft:Draft;tourId:string;onSaved:()=>void}){
 ...
 return <article style={{paddingBlock:20,borderTop:'1px solid var(--imo-border,#d9e2dd)'}}>
  ...
  {draft.stale?<p>تغيرت الصور بعد هذه المسودة. يلزم تحليل الصور الحالية قبل الاعتماد.</p>:<>
   <a href={url} target="_blank" rel="noreferrer"><img src={url} alt={draft.result.furnished?'مسودة مخطط 2D مفروش من صور الشقة':'مسودة الغرف والفتحات من تحليل الصور'} style={{display:'block',width:'100%',maxHeight:720,objectFit:'contain'}}/></a>
   ...
   <details style={{marginTop:16}}><summary>إضافة نسخة مفروشة للمراجعة</summary><p>يمكنك إرفاق مخطط مفروش أُعدّ اعتمادًا على صور الجولة ودليل الغرف. راجع الأثاث والأبواب، ثم أرفق الصورة هنا. يُحفظ إصدار جديد دون استبدال النسخ السابقة. هذا الخيار لإرفاق نسخة يدوية إضافية إلى جانب التوليد التلقائي.</p>
