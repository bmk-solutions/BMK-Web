...
test('100 uploads use bounded parallel transfers, ordered latest-revision commits and one session lookup',async()=>{
 ...
 try{const files=Array.from({length:100},(_,i)=>new File(['data'],i+'.jpg',{type:'image/jpeg'}));const result=await uploadPanoramas({id:'tour',revision},files,0,{onSaved:t=>saved.push(t.revision),onProgress:p=>progress.push(p)});assert.equal(result.uploaded,100);assert.deepEqual(result.failures,[]);assert.deepEqual(committed,files.map(f=>f.name));assert.equal(lookup,1);assert.equal(peak,3);assert.equal(saved.at(-1),104);assert.equal(progress.at(-1).uploadedBytes,400);assert.equal(progress.at(-1).done,100);assert.ok(events.indexOf('init:2.jpg')<events.indexOf('commit:0.jpg'));assert.ok(events.indexOf('init:3.jpg')<events.indexOf('commit:1.jpg'));}finally{globalThis.fetch=original;}
});
