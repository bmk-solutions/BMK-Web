  ...
  try{
   ...
   for(const floor of analysis.floors){
    ...
    for(let offset=0;offset<renderPhotos.length;offset+=groupSize){
     const group=renderPhotos.slice(offset,offset+groupSize),tiles=await Promise.all(group.map(async(item,index)=>({input:await sharp(path.join(directory,item.file)).resize(720,480,{fit:'contain',background:'white'}).png().toBuffer(),left:index%2*720,top:Math.floor(index/2)*480})));
