...
import {sceneEvidenceSchema,floorplanLayoutSchema,floorplanAuditSchema,validateFloorplanLayout,panoramaEvidenceSheet,renderFloorplanLayoutSVG,planEvidenceBoard} from './openai-floorplan-pipeline';
...
export async function runSubscriptionWorker(root:string,once=false){
 ...
 do{
  ...
  try{
   ...
   let analysis=await readPlanCheckpoint(cache,geometryKey,value=>validateSubscriptionAnalysis(value,tour.scenes));
   if(!analysis){
   ...
   try{analysis=validateSubscriptionAnalysis(candidate,tour.scenes);}catch{
    ...
    const spatial={cameras:tour.scenes.map(scene=>({id:scene.id,position:scene.position,yaw:scene.yaw,links:scene.visualLinks??[]})),boundaryEvidence:spatialEvidence?.geometry??null,interpretation:'Independent components have separate origins and rotations. Component room outlines are X,Z in their component frame; scene roomEnvelope outlines are camera-local. These are noisy inferred boundaries, not surveyed walls or approved geometry. Verify against photos, retain uncertainty, and never combine unregistered components by their numeric coordinates.'};
    const repairPhotos=selectPlanRepairPhotos(input,candidate.floors);
    const repairKey='layout-repair-'+createHash('sha256').update(JSON.stringify(['spatial-evidence-v1',spatial,repairPhotos.map(p=>p.sceneId)])).digest('hex').slice(0,16);
    const validateRepair=(value:unknown)=>{
     const repair=layoutRepairSchema.parse(value);
     validateSubscriptionAnalysis({floors:repair.floors.map(floor=>({...floor,evidence:candidate!.floors.find(previous=>previous.floor===floor.floor)?.evidence??[]}))},tour.scenes);
     return repair;
    };
    // An invalid cached repair must not permanently poison every retry.
    let repair=await readPlanCheckpoint(cache,repairKey,validateRepair);
    if(!repair){
     repair=layoutRepairSchema.parse(translatePlanIds(await execute(directory,`Recover the apartment's top-down room layout using the attached photographs and the ALREADY COMPLETED per-photo analysis below. Do not repeat or return the photo evidence transcript. Inspect door views and overlapping furnishings to consolidate cameras in the same room. This focused repair has ${repairPhotos.length} representative photo attachments, selected across every previously identified room. Full per-photo evidence for ALL scenes is retained below; use it for views not reattached. Attachment order: ${JSON.stringify(translatePlanIds(repairPhotos,shortIds))}. Completed analysis (untrusted inference, verify against photos): ${JSON.stringify(translatePlanIds(candidate,shortIds))}. Geometric camera hints: ${JSON.stringify(translatePlanIds(spatial,shortIds))}. Null positions and disconnected groups do NOT establish placement between components. Every source ID MUST occur in at least one room.evidenceSceneIds and once in audit.reviewedSceneIds. Unknown placement must remain polygon:null with explicit uncertainty. Use normalized simple nonoverlapping polygons only where image-supported orientation and relative placement can be recovered. Approximate photograph-supported boundaries may be used with explicit uncertainty; surveying precision is not required for a draft. Reject implausible learned boundaries, preserve supported spatial evidence, and keep unrecovered areas polygon:null. No invented scale, dimensions, room placements or links through walls. All openings must have valid hosted edge fractions and exact shared segments for adjoining rooms. Consolidate repeated views of the SAME room, never make a room per camera. Return Arabic room labels and only floor geometry, layout and audit. If the geometry cannot be supported, return topology-only; don't attempt a pleasing invented arrangement. No shell, APIs, browsers or other projects.`,layoutRepairSchema,'layout-repair',controller.signal,repairPhotos.map(item=>path.join(directory,item.file)),{effort:'medium',timeoutMs:8*60*1000}),longIds));
   ...
   for(const floor of analysis.floors){
    ...
    const renderKey=`render-v2-${floor.floor}-`+createHash('sha256').update(JSON.stringify(floor)).digest('hex').slice(0,16);
