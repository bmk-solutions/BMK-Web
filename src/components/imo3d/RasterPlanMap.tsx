...
export function RasterPlanMap({src,map,sceneIds,current,onSelect,compact=false,zoom=1,heading,onIntent}:{src:string;map:RasterNavigation;sceneIds:string[];current?:string;onSelect:(id:string)=>void;compact?:boolean;zoom?:number;heading?:number;onIntent?:(id:string)=>void}){
 ...
 const center={x:bounds?(bounds.x+bounds.right)/2:map.width/2,y:bounds?(bounds.y+bounds.bottom)/2:map.height/2};
 const w=bounds?bounds.right-bounds.x:map.width,h=bounds?bounds.bottom-bounds.y:map.height;
 const angle=rotation*Math.PI/180,rw=Math.abs(w*Math.cos(angle))+Math.abs(h*Math.sin(angle)),rh=Math.abs(w*Math.sin(angle))+Math.abs(h*Math.cos(angle));
 const rotatedViewBox=`${center.x-rw/2} ${center.y-rh/2} ${rw} ${rh}`;
 const viewBox=bounds?`${bounds.x} ${bounds.y} ${bounds.right-bounds.x} ${bounds.bottom-bounds.y}`:`0 0 ${map.width} ${map.height}`;
 return <><div className="imo-plan-rotation" role="group" aria-label="تدوير المخطط"><button type="button" aria-label="تدوير المخطط يسارًا" disabled={collapsed} onClick={()=>setRotation(r=>(r-15)%360)}><Icon name="rotate-left" size={18}/></button><button type="button" aria-label={collapsed?"إظهار المخطط":"إخفاء المخطط"} aria-expanded={!collapsed} aria-controls={clipId+"-map"} onClick={()=>setCollapsed(value=>!value)}><Icon name={collapsed?"map":"eye"} size={18}/></button><button type="button" aria-label="تدوير المخطط يمينًا" disabled={collapsed} onClick={()=>setRotation(r=>(r+15)%360)}><Icon name="rotate-right" size={18}/></button></div><svg id={clipId+"-map"} data-collapsed={collapsed} className={`imo-raster-map${compact?" is-compact":""}`} style={{display:collapsed?"none":undefined,...(compact?{}:{height:`${60*zoom}dvh`,maxHeight:"none"})}} viewBox={rotation?rotatedViewBox:viewBox} role="img" aria-label="مخطط الشقة؛ اضغط أي مكان للانتقال مباشرة إلى أقرب لقطة" onPointerMove={e=>prepare(e.clientX,e.clientY)} onPointerDown={e=>{prepare(e.clientX,e.clientY);down.current={x:e.clientX,y:e.clientY};}} onPointerCancel={()=>{down.current=null;}} onPointerUp={e=>{
  ...
  <g ref={content} transform={`rotate(${rotation} ${center.x} ${center.y})`}>
