...
export async function subscriptionPlanStatus(tour:Tour){
 try{
  const [workers,jobs]=await Promise.all([workerRows(),cloudQuery<SubscriptionJob[]>('subscription_plan_jobs',`tour_id=eq.${eq(tour.id)}&order=created_at.desc&limit=1`)]);
  const online=engineState(workers,PLANS_WORKER_ID)==='online',provider=onlinePlanProvider(workers),row=jobs[0];
  ...
  return {configured:online,provider,workerOnline:online,job:row?{id:row.id,tourId:row.tour_id,status:expired?'failed':row.status,stage:expired?'انقطع عامل المعالجة؛ أعد المحاولة.':stage,error:expired?'لم تكتمل المعالجة. النسخ السابقة محفوظة.':row.error,createdAt:row.created_at,draftIds:row.draft_ids,result:null}:null,stale:!!row&&row.input_hash!==aiPlanFingerprint(tour.scenes)};
 }catch(error){if(error instanceof CloudError&&['42P01','PGRST205'].includes(error.code))return {configured:false,provider:null,workerOnline:false,job:null,stale:false};throw error;}
}
export async function changeSubscriptionPlan(request:Request,tour:Tour){
 ...
 if(request.method==='DELETE'){
  ...
  if(!status.workerOnline)return fail(planWorkerLabel(null,false),503);
