...
import {planImageURL,type ViewerPlanCache} from "./viewer-plan-cache";
...
export function ViewerFloorPlan({cache,tourId,floor,children,hasInteractivePlan=false,compact=false,current,scenes=[],yaw=0,sceneIds=[],onSelect,onExpand,onIntent}:{cache:ViewerPlanCache;tourId:string;floor:number;children?:ReactNode;hasInteractivePlan?:boolean;compact?:boolean;current?:string;scenes?:Scene[];yaw?:number;sceneIds?:string[];onSelect?:(id:string)=>void;onExpand?:()=>void;onIntent?:(id:string)=>void}){
 ...
 const variant=compact?"mini":"webp";
 const [entry,setEntry]=useState(()=>cache.peek(key,variant)),[error,setError]=useState("");
 // A reviewed architectural plan is the clean plan and opens first; the furnished picture is the alternative.
 const [showInteractive,setShowInteractive]=useState(()=>hasInteractivePlan),[retry,setRetry]=useState(0),[collapsed,setCollapsed]=useState(false);
 const hidden=compact&&hasInteractivePlan;
 useEffect(()=>{
  if(hidden)return;
  let active=true;let timer:ReturnType<typeof setTimeout>;
  const update=async()=>{try{const next=await cache.get(key,variant);if(active){setEntry(next);setError("");}}catch(reason){if(active)setError(reason instanceof Error?reason.message:"تعذر تحميل المخطط.");}finally{if(active)timer=setTimeout(()=>void update(),15000);}};
  void update();return()=>{active=false;clearTimeout(timer);};
 },[cache,key,variant,retry,hidden]);
 ...
 const original=planImageURL(url,job.id,floor),src=entry?.src??planImageURL(url,job.id,floor,variant);
 ...
 return <section className="imo-viewer-draft" aria-label="معاينة مخطط الشقة">
  ...
  {hasInteractivePlan&&<div className="imo-plan-display-modes"><button type="button" aria-pressed={showInteractive} onClick={()=>setShowInteractive(true)}>المخطط النظيف</button><button type="button" aria-pressed={!showInteractive} onClick={()=>setShowInteractive(false)}>المخطط المؤثث</button></div>}
  {showInteractive&&hasInteractivePlan?children:<DraftImage onIntent={onIntent} heading={heading} key={`${job.id}/${floor}`} src={src} original={original} floor={floor} navigation={draft.navigation} current={current} sceneIds={sceneIds} onSelect={onSelect}/>}
 </section>;
}

function DraftImage({src,original,floor,navigation,current,sceneIds,onSelect,heading,onIntent}:{src:string;original:string;floor:number;heading?:number;navigation?:RasterNavigation;current?:string;sceneIds:string[];onSelect?:(id:string)=>void;onIntent?:(id:string)=>void}){
 ...
 return <>
  <div className="imo-viewer-draft-tools" aria-label="أدوات عرض المخطط"><button type="button" aria-label="تصغير المخطط" disabled={zoom===1} onClick={()=>setZoom(z=>Math.max(1,z-.25))}>−</button><button type="button" aria-label="تكبير المخطط" disabled={zoom===3} onClick={()=>setZoom(z=>Math.min(3,z+.25))}>+</button><button type="button" onClick={()=>setZoom(1)}>ملاءمة</button><a href={original} target="_blank" rel="noreferrer">فتح بالحجم الكامل</a></div>
