...
import {hasReviewedArchitecture,type ArchitecturePresentation} from "@/lib/imo3d/architecture-visibility";
import "./interactive-floorplan.css";

const FloorPlan3D=lazy(()=>import("./FloorPlan3D").then(module=>({default:module.FloorPlan3D})));
type Props={tourTitle:string;brandingName?:string;plan:Plan;scenes:Scene[];current:string;yaw:number;position?:Point|null;spatialScale?:Tour["spatialScale"];onSelect:(id:string)=>void;onMeasure?:()=>void;mode?:"2d"|"3d";onModeChange?:(mode:"2d"|"3d")=>void;allowDraft?:boolean;presentation?:ArchitecturePresentation};
export function InteractiveFloorPlan({tourTitle,brandingName,plan,scenes,current,yaw,position,spatialScale,onSelect,onMeasure,mode:controlled,onModeChange,allowDraft=false,presentation}:Props){
