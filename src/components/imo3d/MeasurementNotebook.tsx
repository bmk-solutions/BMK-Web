...
import {createMeasurementStore,type SavedMeasurement} from '../../lib/imo3d/measurement-notebook';
...
export function useMeasurementNotebook(tourId:string,revision:number,sceneIds:string[]){
 const key=`imo3d:measurements:v1:${tourId}:${revision}`;
 const sceneKey=JSON.stringify(sceneIds);
 const store=useMemo(()=>createMeasurementStore(key,new Set<string>(JSON.parse(sceneKey)),()=>typeof window==='undefined'?undefined:window.localStorage),[key,sceneKey]);
