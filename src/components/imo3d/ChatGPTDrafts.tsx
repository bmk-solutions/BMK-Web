...
type Draft={id:string;floor:number;stale:boolean;created_at:string;result:{furnished?:{reviewNotes:string};audit:{verdict:string;issues:string[];limitations:string[]}}};
function DraftCard({draft,tourId,onSaved}:{draft:Draft;tourId:string;onSaved:()=>void}){
 ...
 return <article style={{paddingBlock:20,borderTop:'1px solid var(--imo-border,#d9e2dd)'}}>
  ...
  {draft.stale?<p>تغيرت الصور بعد هذه المسودة. يلزم تحليل الصور الحالية قبل الاعتماد.</p>:<>
   ...
   <details style={{marginTop:16}}><summary>إضافة نسخة مفروشة للمراجعة</summary><p>اطلب من ChatGPT رسم المخطط مفروشًا اعتمادًا على صور الجولة ودليل الغرف. راجع الأثاث والأبواب، ثم أرفق الصورة هنا. يُحفظ إصدار جديد دون استبدال النسخ السابقة. التوليد داخل المحادثة وليس تلقائيًا عند رفع الصور.</p>
...
export function ChatGPTDrafts({tourId}:{tourId:string}){
 ...
 return <section><h4>مسودات ChatGPT</h4><p>صور الشقة ← مراجعة الغرف والأثاث ← مخطط 2D مفروش للمراجعة. لكل شقة توزيعها وأثاثها الخاص.</p>{!drafts.length&&<p>بعد ربط حسابك من الإعدادات، تظهر هنا المسودات التي يحفظها ChatGPT لهذه الجولة.</p>}{error&&<p role="status">{error}</p>}{drafts.map(draft=><DraftCard key={draft.id} draft={draft} tourId={tourId} onSaved={()=>setRevision(value=>value+1)}/>)}</section>;
}
